...
export class PageComponent extends BaseComponent {
    ...
    onDrop(event) {
        ...
        if (this.dragTarget && this.dragTarget !== this.dropTarget) {
            ...
            const scrElement = this.dragTarget.getBoundingRect();
            this.dragTarget.removeFrom(this.element);
            this.dropTarget.attach(this.dragTarget, dropY < scrElement.y ? 'beforebegin' : 'afterend');
        }
    }
    addChild(section) {
        ...
        item.setOnDragStateListener((target, state) => {
            switch (state) {
                ...
                case 'leave':
                    ...
                    break;
                default:
                    throw new Error(`unsuppoted state: ${state}`);
            }
        });
    }
    updateSection(state) {
